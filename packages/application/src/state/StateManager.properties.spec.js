...
describe('StateManager Properties', () => {
  it('Property 27: State Change Notifies Subscribers', () => {
    fc.assert(
      fc.property(fc.dictionary(fc.string(), fc.anything()), fc.dictionary(fc.string(), fc.anything()), (initial, update) => {
  ...
  it('Property 30: Middleware Called on State Changes', () => {
    fc.assert(
      fc.property(fc.dictionary(fc.string(), fc.anything()), (update) => {
        const store = new StateManager();
        const middleware = vi.fn((nextState, prevState) => nextState);
        store.use(middleware);

        store.setState(update);

        expect(middleware).toHaveBeenCalledTimes(1);
        const [nextState, prevState] = middleware.mock.calls[0];
        for (const [k, v] of Object.entries(update)) {
